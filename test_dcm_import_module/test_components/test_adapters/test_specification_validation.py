"""Test module for `SpecificationValidationAdapter`."""

import pytest
from dcm_common.services import APIResult

from dcm_import_module.components import SpecificationValidationAdapter
...
def _adapter(url):
    return SpecificationValidationAdapter(url)
...
def test_run(
    adapter: SpecificationValidationAdapter,
    request_body,
    target,
    report,
    service_app,
):
    """Test method `run` of `SpecificationValidationAdapter`."""
...
def test_success(
    adapter: SpecificationValidationAdapter, request_body, target, service_app
):
    """Test property `success` of `SpecificationValidationAdapter`."""
    adapter.run(request_body, target, info := APIResult())
