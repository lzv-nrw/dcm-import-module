"""Test module for `BuildAdapter`."""

import pytest
from dcm_common.services import APIResult

from dcm_import_module.components import BuildAdapter
...
def _adapter(url):
    return BuildAdapter(url)


@pytest.fixture(name="target")
def _target():
    return {"path": "ie/59438ebf-75e0-4345-8d6b-132a57e1e4f5"}
...
def test_run(
    ...
    """Test method `run` of `BuildAdapter`."""
...
def test_success(adapter: BuildAdapter, request_body, target, service_app):
    """Test property `success` of `BuildAdapter`."""
    adapter.run(request_body, target, info := APIResult())
